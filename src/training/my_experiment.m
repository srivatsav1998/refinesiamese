% -------------------------------------------------------------------------------------------------
function [net, stats] = my_experiment(imdb_video, varargin)
...
% -------------------------------------------------------------------------------------------------
    ...
    opts.expDir = 'trained_data'; % where to save the trained net
    ...
    opts.pairsPerVideoTrain =  5; % avg number of pairs per training video (3862) %25 earlier
    opts.pairsPerVideoVal =  10; % avg number of pairs per validation video (555) %20 earlier
    opts.negatives = 0.2; % fraction of negative pairs taken from different videos %0 earlier
    opts.hardNegatives = 0.3; % fraction of negative pairs taken from same video %0 earlier
    opts.randomSeed = 0;
...
%    Load dataset statistics for data augmentation
    ...
    % -------------------------------------------------------------------------------------------------
%     net = gen_network(opts);
    net = gen_network();
%     net.print({'exemplar', [127 127 3 8], 'instance', [255 255 3 8]}, 'Format', 'dot');
...
% -------------------------------------------------------------------------------------------------
    ...
    fprintf(f, net_dot);
    fclose(f);
    [~, ~] = system(['dot -Tpdf ' dot_file ' -o' pdf_file]);
end
...
% -------------------------------------------------------------------------------------------------
    sizes = net.getVarSizes({'exemplar', [opts.exemplarSize*[1 1] 3 256], ...
                             'instance', [opts.instanceSize*[1 1] 3 256]});
    tmpl_sz = sizes{net.getVarIndex('branch1_rfmod2_outrcu1_sum1')}(1:2);
    rfs = net.getVarReceptiveFields('exemplar');
    tmpl_stride = rfs(net.getVarIndex('branch1_rfmod2_outrcu1_sum1')).stride(1);
    assert(all(rfs(net.getVarIndex('branch1_rfmod2_outrcu1_sum1')).stride == tmpl_stride));
end
